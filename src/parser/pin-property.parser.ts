...
export class PinPropertyParser implements CustomPropertyParser {
    ...
    private static readonly _ATTRIBUTE_PARSERS: {
        ...
        "PinName": (p: PinProperty, value: string) => { p.name = BlueprintParserUtils.parseString(value); },
        ...
        "DefaultObject": (p: PinProperty, value: string) => {
            ...
            const matches = /(?<=\.)[^_C]*/g.exec(defaultObject);
            if(!matches[0]) { return; }
            p.defaultValue = removeInsignificantTrailingZeros(matches[0]);
        },
