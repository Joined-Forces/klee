        ...
        "DefaultValue": (p: PinProperty, value: string) => {
            ...
            switch (p.category) {
                case PinCategory.float:
                    p.defaultValue = removeInsignificantTrailingZeros(BlueprintParserUtils.parseString(value));
                    break;
                case PinCategory.bool:
                    p.defaultValue = (BlueprintParserUtils.parseString(value).toLowerCase() === "true");
                    console.log(value.toLowerCase());
                    console.log(typeof p.defaultValue);
                    console.log(p.defaultValue);
                    break;
                // case PinCategory.int:
                // case PinCategory.int64:
                // case PinCategory.byte:
                //     p.defaultValue = BlueprintParserUtils.parseString(value); break;
                case PinCategory.struct:
                    if (p.subCategoryObject === StructClass.VECTOR ||
                        p.subCategoryObject === StructClass.ROTATOR) {
                        ...
                        for (let i = 0; i < defaultValues.length; i++) {
                            defaultValues[i] = `${LABELS[i]} ${defaultValues[i]}`;
                        }

                        p.defaultValue = defaultValues;
                    }
                    break;
                default:
                    p.defaultValue = BlueprintParserUtils.parseString(value);
                    break;
            }
        },
